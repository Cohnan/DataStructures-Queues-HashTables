...
public class THLinProbTest extends TestCase{
	 ...
	 * Atributos 
	 */
	private LinProbTH<String, Integer> tabla;
	...
	private void setUpEscenario(int n, int max) {
		tabla = new LinProbTH<String, Integer>(max);
		for (int i = 0; i < n; i++) {
	...
	public void testArregloDinamico() {
		for (int n = 0; n < numeroEscenarios; n++) {
			...
			assertTrue("El arreglo deberia tener tamano " + n, tabla.darTamano() == n);
		}
	}
	
	/**
	 * Prueba el metodo put
	 */
	public void testPut() {
		int nAgregar = 500;
		...
		for (int n = 0; n <= numeroEscenarios; n++) {
			...
			for (int i = 0; i < nAgregar; i++) tabla.put("Nuevo elemento " + i, i);
			assertTrue("Escenario: " + n + ". El arreglo deberia tener " + (n + nAgregar) + " elementos."
					+ " Pero tiene " + tabla.darTamano(), arreglo.darTamano() == (n + nAgregados));
